...
_.forEach(creditScore, function (credit) {
    // generate new object on each loop at the top
        ...
        _.forEach(remarks, function (remark) {
            ...
            _.forEach(utilization, function (util) {
                ...
                _.forEach(hardInquiries, function (inq) {
                    ...
                    // looping by reference so persist a copy
...
function applyApprovals(data) {
    return _.map(data, function (d) {
        // Excellent credit score is approved, no question asked
...
var json = JSON.stringify(predictionDataWithApprovals);
var fs = require('fs');
fs.writeFile('predictionDataWithApprovals.json', json, 'utf8', function (response) {
